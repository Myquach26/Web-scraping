...
rD$open()
rD$navigate("https://www.investing.com/indices/stoxx-50-volatility-vstoxx-eur")
webElem <- rD$findElement(using = "css selector", value = "span[data-test='instrument-price-last']")
vix_value <- webElem$getElementText()[[1]]
print(vix_value) #shows current value for VSTOXX index
#The VSTOXX indices track real-time options prices on the EURO STOXX 50® benchmark, thus reflecting market expectations of future volatility in Eurozone stocks

#properly close seleniun session
rD$close()
rD$quit()
